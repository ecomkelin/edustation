# EduStation 项目 AGENTS.md

> 本文件为校外培训机构管理系统的核心开发指引，供 Codex (CC) 协作开发时读取。  
> 每次会话都用得到的核心约定在此；具体数据模型 / 模块设计见 [docs/Codex/](docs/Codex/) 子目录。
...
**唯一例外**：已经在 docs/Codex/ 里的"历史整改记录"（如 Org.type 整改 2026-06-15）保留作为决策溯源，但**今后不要再写新的兼容代码**。
...
- 机构与字典 → [docs/Codex/data-models-org.md](docs/Codex/data-models-org.md)
- 用户/职位/权限 → [docs/Codex/data-models-user.md](docs/Codex/data-models-user.md)
- 学生 → [docs/Codex/data-models-student.md](docs/Codex/data-models-student.md)
- 课程与开班 → [docs/Codex/data-models-course.md](docs/Codex/data-models-course.md)
- 报名/排课/消课 → [docs/Codex/data-models-enrollment.md](docs/Codex/data-models-enrollment.md)
- 订单/课包 → [docs/Codex/data-models-order.md](docs/Codex/data-models-order.md)
- 积分/宠物 → [docs/Codex/data-models-points-pet.md](docs/Codex/data-models-points-pet.md)
- 招生/潜客/试听 → [docs/Codex/data-models-recruit.md](docs/Codex/data-models-recruit.md)
- 财务账本/流水 → [docs/Codex/data-models-finance.md](docs/Codex/data-models-finance.md)
- 文件存储 → [docs/Codex/data-models-storage.md](docs/Codex/data-models-storage.md)
- 人脸门禁 → [docs/Codex/data-models-access.md](docs/Codex/data-models-access.md)
...
- **完整路由索引（按编号 R-MMPP 排序）**：[docs/Codex/routes-server.md](docs/Codex/routes-server.md)
...
2. **复用现有字段** (老模型)
   - `CourseInstance` 复用 `deletedAt` (AGENTS.md §8.1 软删) — 加 `recover` 端点
...
- 模块组织：领域分组 + 垂直切片
- 内存型知识（一次性教训/坑点）→ 写到 `~/.Codex/projects/.../memory/`，不塞回 AGENTS.md

---

## 14. 何时读 docs/Codex/ 下的文件
...
| 场景 | 读哪个文件 |
|---|---|
| 改机构 / Org / 推广 / 业态分类 / Category 字典 | [docs/Codex/data-models-org.md](docs/Codex/data-models-org.md) |
| 改用户 / 登录 / 权限码 / 职位 / 多租户身份 | [docs/Codex/data-models-user.md](docs/Codex/data-models-user.md) |
| 改学生 / 家长切换 / 监护人 | [docs/Codex/data-models-student.md](docs/Codex/data-models-student.md) |
| 改学科 / 课程产品 / 开班 / 排课计划 | [docs/Codex/data-models-course.md](docs/Codex/data-models-course.md) |
| 改报名 / 排课 / 考勤 / 作品 / 消课选包 | [docs/Codex/data-models-enrollment.md](docs/Codex/data-models-enrollment.md) |
| 改订单 / 课包 / 支付 / 赠课 | [docs/Codex/data-models-order.md](docs/Codex/data-models-order.md) |
| 改积分 / 宠物 / 喂养 / 商店 | [docs/Codex/data-models-points-pet.md](docs/Codex/data-models-points-pet.md) |
| 改招生 / 潜客 / 家长业务档案 / 试听 / 转化 | [docs/Codex/data-models-recruit.md](docs/Codex/data-models-recruit.md) |
| 改财务账本 / 流水 / 转账 / 收支原因 / 汇总 | [docs/Codex/data-models-finance.md](docs/Codex/data-models-finance.md) |
| 改文件上传 / File / Storage 驱动 / 引用追踪 | [docs/Codex/data-models-storage.md](docs/Codex/data-models-storage.md) |
| 改人脸识别 / 门禁 / 接送授权 | [docs/Codex/data-models-access.md](docs/Codex/data-models-access.md) |
| 改 AI 助手 / RAG / 会话持久化 / 平台客服永久会话 (R-2830) | [docs/Codex/ai-assistant.md](docs/Codex/ai-assistant.md) |
| 改经营看板 / 报表 / 聚合管道 | [docs/Codex/dashboards.md](docs/Codex/dashboards.md) |
| 改 推送通知 / 通知模板 / 用户偏好 / 渠道分发 / inbox / 红点 (MM=40 v0.9) | [docs/Codex/data-models-notification.md](docs/Codex/data-models-notification.md) |
| 改 / 加 / 删 / 查后端 API 路由（编号、Auth、跨模块） | [docs/Codex/routes-server.md](docs/Codex/routes-server.md) |
| 上云 / 部署 / 扩容 / 备份 / CI/CD（云厂选型、Nginx、PM2、迁移） | [docs/Codex/deploy-guide.md](docs/Codex/deploy-guide.md) |
| 跑 client 项目 / HBuilderX / 微信开发者工具 / App 真机调试 / manifest 配置 | [docs/Codex/client-test-guide.md](docs/Codex/client-test-guide.md) |

## 15. 新增知识到本仓库的约定

- **新数据模型 / 业务域**：在 `docs/Codex/data-models-<domain>.md` 新建或追加；不要塞回 AGENTS.md。
- **新非模型知识**（如支付对接 / 税务规则 / 外部集成）：建 `docs/Codex/<topic>.md`。
- **新通用约定 / 命名 / 横切规则**（每次都用）：加进 AGENTS.md 对应章节。
- 新子文件顶部必须有 `> 何时读这个文件：...` + 1 行摘要，方便模型快速判断是否对路。
- 新子文件加入 §14 "何时读" 表。
- **一次性坑点 / 调试教训**（如某字段类型 bug、某 API 误用）：写到 `~/.Codex/projects/.../memory/` 对应文件，不写进 docs/Codex/。
- **改任何 `*.routes.js` 文件**：必须在 `routes-server.md` 同步加/改/废弃行；新增端点取下一个空 PP（不重排、不填空），废弃端点 R 号保留移入 §4.1；详见 §14 入口。

## 16. 维护规则

每次重大架构变更或决策后，更新本文件或对应 `docs/Codex/` 子文件，并通知协作者。
