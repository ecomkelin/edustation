...
> **一行摘要**：6 块经营看板（经营总览 / 课消与课表 / 教室与排课利用率 / 老师产能与绩效 / 积分与家长活跃 / **任务概览**），所有指标按 x-org-id 隔离。
...
| 6 | **任务概览**（2026-08-06 P3.2 立项） | `GET /api/v1/reports/task-overview` | 总任务数 / 未完结 / 已逾期 / 完成率；按 tag Top 20 分布（总数/已完成/逾期/完成率）+ 按 status 桶；累计型指标（与 range 无关），60s 进程内 `withCache` 缓存 |
